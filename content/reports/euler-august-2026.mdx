...
The reason is a governance decision with a date on it rather than anything about pricing. Euler's `ProtocolConfig` contract sets the share of the interest fee the protocol receives, and on both Ethereum and Monad that share was set to **zero on 12 May 2026**, down from 50% [4]. What the protocol still books comes from two narrow places: **472 Ethereum vaults whose curator has never set a payout address**, where the protocol collects the entire interest fee by default, and seven vaults carrying an explicit override. All four of Euler's largest Ethereum markets sit outside both, and pay it nothing.
